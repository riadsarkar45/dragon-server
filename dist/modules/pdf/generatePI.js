...
exports.generatePI = void 0;
const pdfkit_1 = __importDefault(require("pdfkit"));
const generatePI = async (req, reply) => {
    ...
    reply
        ...
        .header("Content-Disposition", `attachment; filename=${encodeURIComponent(buyerName || "PI")}.pdf`)
        .send(doc);
    try {
        // Header
        ...
        // Footer
        doc.moveDown(2);
        doc.fontSize(9).font("Helvetica").text(`This Proforma Invoice is issued in accordance with the terms referenced in ` +
            `LC No. 0000265250400284 and Proforma Invoice No. ${PI_NUMBER} dated ${PI_DATE}.\n` +
            `All shipments are subject to standard export compliance.`, { width: pageWidth - 80 });
        doc.end();
    }
    catch (err) {
